...
def runflye(bc):
    #print ('Running flye...')
    comm='flye --nano-hq {0}/{1}.fastq.gz  --meta -o {0}/flye_{1} -t {2}'.format(outdir,bc,threads)
    #print (comm)
    subprocess.getoutput(comm)

def rungetspe(bc):
    #print ('Run getting spe...')
    comm='getspe1.py {0}/{1}_cenout.txt'.format(outdir,bc)  #20230609
    subprocess.getoutput(comm)
    comm='seqkit grep -f {0}/{1}_names.txt /opt/csequences.fna -j 5 -o {0}/{1}_refs_all.fasta'.format(outdir,bc)
    subprocess.getoutput(comm)
    comm='cat {0}/{1}_refs_all.fasta | seqkit seq -m 500000 -w0 > {0}/{1}_refs.fasta'.format(outdir,bc)
    subprocess.getoutput(comm)
    comm='minimap2 {0}/{1}_refs.fasta {0}/{1}.fastq.gz -t {2} > {0}/{1}.paf'.format(outdir,bc,threads)
    subprocess.getoutput(comm)
    comm='getseqs.py {0}/{1}.paf 1000 {0}/{1}_names.txt'.format(outdir,bc)
    subprocess.getoutput(comm)
    ###20230609
    if not os.path.exists('{0}/{1}_Dref.fasta'.format(outdir,bc)):
        if not os.path.exists('{0}/{1}_spe'.format(outdir,bc)):
            os.mkdir ('{0}/{1}_spe'.format(outdir,bc))
        comm='cp {0}/flye_{1}/assembly.fasta {0}/{1}_spe/cluster.fasta'.format(outdir,bc)
        subprocess.getoutput(comm)
    else:
        comm='assigncontigs.py {0}/flye_{1}/assembly.fasta {0}/{1}_Dref.fasta {0}/{1}_spe'.format(outdir,bc)
        stdout=subprocess.getoutput(comm)

def runresfinder(bc):
    #pspes=['Campylobacter coli','Campylobacter jejuni','Enterococcus faecalis','Enterococcus faecium','Escherichia coli','Helicobacter pylori','Kebsiella']
    ...
    spe4p=''
    cwd=os.getcwd()
    os.chdir(os.path.join(outdir,'{0}_spe'.format(bc)))
    myfiles=[x for x in os.listdir() if '.fasta' in x and 'old' not in x]
    #print (myfiles)
    for i in myfiles:
        if 'cluster1.' in i:
            name='c1'
            comm='grep "Cluster" cluster1.txt'
            stdout=subprocess.getoutput(comm)
            spe4p=stdout.split(': ')[1]

        if 'cluster2.' in i:
            name='c2'
            comm='grep "Cluster" cluster2.txt'
            stdout=subprocess.getoutput(comm)
            spe4p=stdout.split(': ')[1]

        if 'cluster.' in i:
            name='c'
            comm='grep "Cluster" cluster.txt'
            stdout=subprocess.getoutput(comm)
            spe4p=stdout.split(': ')[1]
            #print (spe4p)
        if 'other' in i:
            name='o'
        if spe4p in psped.keys():
            comm='python3 -m resfinder -o resfinder{0} -l 0.6 -t 0.9 --acquired -ifa {1} -c --species "{2}"'.format(name,i,psped[spe4p])
            #print (comm)
        else:
            comm='python3 -m resfinder -o resfinder{0} -l 0.6 -t 0.9 --acquired -ifa {1}'.format(name,i)
        stdout=subprocess.getoutput(comm)
        pfile='resfinder{0}/PointFinder_results.txt'.format(name)
        rfile='resfinder{0}/ResFinder_results_tab.txt'.format(name)
        #print (pfile)
        ...
        if spe4p in intrinsic_res.keys():
            ind={'Phenotype':intrinsic_res[spe4p]}
...
def main():
    ...
    comm='updatespe.py {0}'.format(outdir)
    print (comm)
    subprocess.getoutput(comm)

    comm='getresults.py -i {0}'.format(outdir)
    print (comm)
    subprocess.getoutput(comm)
